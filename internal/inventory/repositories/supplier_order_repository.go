...
func (r *SupplierOrderRepository) Create(order *models.SupplierOrder) error {
	query := `
		INSERT INTO supplier_orders (supplier, quantity, gender, gamme, order_date, note, created_by)
		VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, created_at, updated_at
    `
	return r.db.QueryRowx(query, order.Supplier, order.Quantity, order.Gender, order.Gamme, order.OrderDate, order.Note, order.CreatedBy).
...
func (r *SupplierOrderRepository) List() ([]models.SupplierOrder, error) {
	...
	err := r.db.Select(&orders, `
		SELECT id, supplier, quantity, gender, gamme, order_date, note, created_by, created_at, updated_at
