...
func main() {
	...
	preRegistrationHandler := inventoryHandlers.NewPreRegistrationHandler(preRegistrationRepo, storageSvc)
	...
	router.Use(func(c *gin.Context) {
		...
		c.Header("Content-Security-Policy",
			"default-src 'self'; "+
				...
				"connect-src 'self' https://api-lunetterie.universearch.com https://lunetterie-front.onrender.com http://localhost:8000 http://127.0.0.1:8000; "+
	...
	v1 := router.Group("/api/v1")
	{
		...
		inventory.Use(authMiddleware.RequireAuth(identityAuthSvc))
		{
			...
			receptionCommands := inventory.Group("/reception-commands")
			{
				...
				receptionCommands.POST("/cases/:caseId/boxes/upload-photo", authMiddleware.RequireRoles(1, 2, 3, 8, 11, 12), preRegistrationHandler.UploadBoxPhoto)
				receptionCommands.POST("/cases/:caseId/boxes/:boxId/photos", authMiddleware.RequireRoles(1, 2, 3, 8, 11, 12), preRegistrationHandler.UpdateBoxPhotos)
